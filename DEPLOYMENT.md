...
Upload the **contents** of this deployment package directly to the publishing
branch root. Do not upload a parent build folder around the website.
...
After uploading, confirm that `/assets/css/master.css` and
`/assets/js/master.js` open successfully on the published domain. If the
homepage appears as unformatted text, the `assets` directory is not at the same
repository-root level as `index.html`.
