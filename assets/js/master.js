(() => {
  ...
  function initPersonalization(source) {
    ...
    const sourceText =
      `${source.utm_source} ${source.utm_medium} ${source.utm_campaign} ${source.referrer} ${location.search}`.toLowerCase();
    let variant = "default";
    if (/ai|automation|dashboard|workflow/.test(sourceText)) variant = "ai";
    else if (/cyber|security|firewall|vpn/.test(sourceText))
      variant = "security";
    else if (/managed.?it|infrastructure|support|cloud/.test(sourceText))
      variant = "services";
    else if (/linkedin|social/.test(sourceText)) variant = "leadership";
    ...
    const messages = {
      ...
      ai: {
        eyebrow: "Practical AI and workflow automation",
        line: "Improve reporting, document handling, customer engagement, and repetitive workflows with bounded automation and human escalation.",
      },
      security: {
        eyebrow: "Secure networks and controlled cloud change",
        line: "Connect Microsoft 365, firewall, VPN, endpoint, identity, and migration requirements through a clearly scoped security approach.",
      },
      services: {
        eyebrow: "UAE-based managed IT and cloud services",
        line: "Strengthen user support, Microsoft 365, infrastructure, connectivity, and operational handover with direct ALZA delivery support.",
      },
      default: {
        eyebrow: "Migration software and UAE technology services",
        line: "Run Microsoft 365 migration workloads from infrastructure you control, then connect the outcome to practical cloud, managed IT, security, and automation support.",
      },
    };
  ...
  function fallbackEmail(form) {
    ...
    const recipient =
      data.get("interest") === "Product support"
        ? config.site?.supportEmail || "support@alzatotaltech.com"
        : config.site?.contactEmail || "sales@alzatotaltech.com";
  ...
  function initForms(source) {
    qsa(".lead-form").forEach((form) => {
      ...
      const requestedInterest = new URLSearchParams(location.search).get(
        "interest",
      );
      const interestAliases = {
        "m365-demo": "Microsoft 365 Migration Suite",
        quotation: "Microsoft 365 Migration Suite",
        "managed-it": "Managed IT support",
        infrastructure: "Managed IT support",
        cybersecurity: "Network and cybersecurity",
        "custom-app": "AI automation or custom application",
      };
      fieldValue(
        form,
        "interest",
        interestAliases[requestedInterest] || requestedInterest || "",
      );
      ...
      form.addEventListener("submit", async (event) => {
        ...
        emit("lead_form_submit", trackingContext);
        if (!valid(endpoint)) {
          if (status) {
            status.textContent =
        ...
        try {
          ...
          emit("generate_lead", trackingContext);
          const redirect =
          ...
          if (status) {
            status.textContent = `We could not send the form. Email ${config.site?.contactEmail || "sales@alzatotaltech.com"} instead.`;
