  ...
  return <AnimatePresence>{open && <motion.div className="modal react-modal" role="dialog" aria-modal="true" aria-labelledby="lead-modal-title" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={(e)=>{if(e.target===e.currentTarget) onClose()}}>
    <motion.div className="modal-card" initial={{opacity:0,y:22,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:12,scale:.99}} transition={{duration:.22}}>
      ...
      <form className="lead-form" onSubmit={submit}>
        <div className="form-grid"><label>Name<input ref={first} name="name" required/></label><label>Work email<input type="email" name="email" required/></label><label>Company<input name="company"/></label><label>Phone<input name="phone"/></label></div>
        <label>Service or project type<select name="service" defaultValue=""><option value="">Select one</option><option>Home or office IT setup</option><option>Network or IT infrastructure</option><option>Cloud or Microsoft 365</option><option>Cybersecurity</option><option>Managed IT support</option><option>Website design or development</option><option>Microsoft 365 migration</option><option>AI or workflow automation</option><option>Other</option></select></label>
        <label>What can we help with?<textarea name="message" rows={5} required/></label><button className="btn" type="submit">Submit enquiry</button>{status && <p className="form-status">{status}</p>}
      </form>
    </motion.div>
