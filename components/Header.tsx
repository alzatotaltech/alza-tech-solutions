...
const nav = [
  ["Home", "/"], ["Products", "/products/"], ["Solutions", "/solutions/"],
  ["Services", "/services/"], ["Pricing", "/pricing/"], ["Resources", "/resources/"],
...
export function Header() {
  ...
  const active = (href: string) => href === "/" ? pathname === "/" : (pathname || "/").replace(/\/+$/, "") === href.replace(/\/+$/, "");
  ...
  return <motion.header className="site-header" initial={{opacity:0,y:-18}} animate={{opacity:1,y:0}} transition={{duration:.5,ease:[.22,1,.36,1]}}>
    ...
    <div className="container nav-wrap">
      <Link aria-label="ALZA Total Tech Solutions home" className="brand" href="/">
        ...
        <span className="brand-copy brand-copy-static-v76"><strong>ALZA</strong><span>Total Tech Solutions FZ-LLC</span><small>IT Infrastructure • Cloud • Cybersecurity • Software</small></span>
      </Link>
      ...
      <nav aria-label="Primary" className={`desktop-nav${open ? " open" : ""}`} id="site-nav">
        ...
        <ThemeToggle/>
        <button className="btn btn-sm nav-lead-button" type="button" onClick={openLead}>Talk to ALZA</button>
      </nav>
    </div>
  </motion.header>;
}
